...
void app_sec_init()
{
	...
	app_sec_env.peer_pairing = false;//配对
	app_sec_env.peer_encrypt = false;	//加密
	app_sec_env.bond_lost = false;//邦定失败
	app_sec_env.pairing_fail = false;//配对失败
}
...
void app_sec_remove_bond(void)//解除绑定
{
                                        ...
                                        ke_task_id_t const src_id)
{
	...
	UART_PRINTF("%s \r\n",__func__);
	app_sec_env.peer_encrypt = true;//加密
    // LTK value
    struct gapc_ltk ltk;
    // Length
