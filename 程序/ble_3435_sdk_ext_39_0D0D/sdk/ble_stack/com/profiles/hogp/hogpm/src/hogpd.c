...
const struct attm_desc hids_att_db[HOGPD_IDX_NB] =
{
   ...
   // [HOGPD_IDX_REPORT_MAP_VAL]                  = {ATT_CHAR_REPORT_MAP, PERM(RD, ENABLE), PERM(RI, ENABLE), HOGPD_REPORT_MAP_MAX_LEN},
    ...
    [HOGPD_IDX_BOOT_MOUSE_IN_REPORT_VAL]        = {ATT_CHAR_BOOT_MOUSE_IN_REPORT, (PERM(RD, ENABLE) | PERM(WRITE_REQ, ENABLE) | PERM(NTF, ENABLE)), PERM(RI, ENABLE), HOGPD_BOOT_REPORT_MAX_LEN,},
    ...
    [HOGPD_IDX_REPORT_VAL]                      = {ATT_CHAR_REPORT, (PERM(RD, ENABLE) | PERM(WRITE_REQ, ENABLE) | PERM(NTF,ENABLE)), PERM(RI, ENABLE), HOGPD_REPORT_MAX_LEN},
        ...
        for (report_idx = 0; report_idx < params->cfg[svc_idx].report_nb; report_idx++)
        {
            ...
            switch (params->cfg[svc_idx].report_char_cfg[report_idx] & HOGPD_CFG_REPORT_FEAT)
            {
                ...
                case HOGPD_CFG_REPORT_IN:
                {
                    ...
                    perm = PERM(RD, ENABLE) | PERM(NTF, ENABLE) | PERM(WRITE_REQ, ENABLE);
        ...
        *report_idx = 0;
        ...
        else
        {
            ...
            for(*report_idx = 0 ; (*report_idx < hogpd_env->svcs[*svc_idx].nb_report) ; (*report_idx)++)
            {
                ...
                if(handle <= hdl_cursor)
                {
                    *att_idx = HOGPD_IDX_REPORT_END-1 - (hdl_cursor - handle - 1);
                    break;
                }
